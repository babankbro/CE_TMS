...
The Next.js 16 (App Router) + TypeScript + Tailwind app lives at the **repo root** (`app/`,
`components/`, `lib/`, `data/`, `package.json`) so Vercel deploys it without a custom Root Directory.
- `seed_json.py` — re-parses `example/*.pdf` (pdfplumber) into `data/dataset.seed.json`.
...
# Regenerate seed data from the PDFs (needs: pip install pdfplumber)
python seed_json.py
```
...
### Conflict engine (`lib/conflicts.ts`) — the core logic, fully unit-tested
...
### Storage + concurrency (`lib/storage.ts`, `dataStore.ts`, `api.ts`, `app/api/data/route.ts`)
...
- `AGENTS.md` warns that this Next.js major version differs from older docs; check
